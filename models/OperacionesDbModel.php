...
class OperacionesDbModel
{
  ...
  public function buscarArticulos($idRevista) {
    $qArticulos = "SELECT * FROM articulos WHERE doc_revista = $idRevista AND estado != 'INACTIVO' ";
    $result = $this->conn->query($qArticulos);
  ...
  public function buscarArticuloId($idArt) {
    $qArticulo = "SELECT * FROM articulos WHERE doc_articulo = $idArt AND estado != 'INACTIVO' ";
    $result = $this->conn->query($qArticulo);
